...
public class WatchDog {
	...
	private long startTS;
	  ...
	  public WatchDog(Queue<LogEvent> log, PluginConfig config) {
	      ...
	      LogEvent le = new LogEvent("INFO",ControllerEngine.pluginID,"WatchDog timer set to " + config.getWatchDogTimer() + " milliseconds");
		  log.offer(le);
	  }
	...
	class WatchDogTask extends TimerTask {
	    public void run() {
	    	...
	    	long runTime = System.currentTimeMillis() - startTS;
			 LogEvent le = new LogEvent("WATCHDOG",ControllerEngine.pluginID,"Plugin Uptime " + String.valueOf(runTime) + "ms");
			 log.offer(le);
